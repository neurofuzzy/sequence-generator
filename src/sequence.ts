...
export class Sequence {
  ...
  static updateSeedAll = (seed: number) => {
    for (const alias in Sequence.sequences) {
      Sequence.sequences[alias].updateSeed(seed);
    }
  };
  ...
  static fromStatement(
    ...
    let alias = "";
    let accumIndex = 2;
    let sequenceSeed = seed; // Use provided seed as default
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (tokens[i] === "seed" && i + 1 < tokens.length) {
        seedIndex = i;
        break;
      }
    }
    ...
    if (asIndex !== -1 && asIndex + 1 < tokens.length) {
      ...
      if (asIndex + 2 < aliasEndIndex && Sequence.accumulators.indexOf(tokens[asIndex + 2]) === -1) {
        ...
        // For now, just use the immediate next token as alias
      }
    } else {
